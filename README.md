...
| 本站额度用完 | `429` |
| 全站闸门繁忙（反代侧突发限流） | `429` |
| 同一个 IP 打太快（nginx 边缘限流） | `429` |
| Key 无效 / 没带 / 被停用 | `401` |
| 上游失败（重试 3 次仍不行） | `502` |

> ⚠️ **所有 `429` 都是 JSON，而且 `cause` 会告诉你"是谁的限额"** ——
> 这一条是修出来的：nginx 边缘限流以前回的是 **`503` + 一页 HTML**，
> 按 `{success,cause}` 解析的客户端直接炸，重试库还会把它当成"服务器挂了"。
> 现在三层限流全都回 429 + JSON，并且分得清是谁：
>
> | `cause` 里出现 | 是谁的限额 | 怎么办 |
> |---|---|---|
> | `reverse-proxy edge` | 你**这个 IP** 的突发限流（20 req/s，瞬时最多 60） | 降速重试 |
> | `reverse-proxy`（没有 edge） | **全站**共享的突发限流（默认 600/分钟） | 等几秒重试 |
...
| 层 | 位置 | 默认 | 管什么 |
|---|---|---|---|
| ① 每 IP 突发 | nginx（`limit_req`） | **20 req/s**，burst 60 | 防单机刷爆。3 把 Key 的池子一共才 900/分钟，单个 IP 给 20/s 已经比池子本身还宽 |
| ② 全站突发 | 服务进程（`QQBOT_PROXY_RATE`） | **600 / 分钟** | 防"很多 IP 各打一点"把 Hypixel 池子打光。按池子容量给：3 把 × 300 × 2/3 |
| ③ 每 Key 额度 | 服务进程（`/apikey rate`） | **225 / 分钟**（按响应体积加权） | **真正的公平分配**。这一层才是"你的额度" |

③ 是唯一跟调用方有关的。①② 是保护池子的阀门，调它们找管理员改
`QQBOT_PROXY_RATE` / nginx 的 `bskweb_req`。

> 这一层以前是**错的**：反代跟 `/api/player`（要真打 Hypixel/Urchin 的重接口）
> 共用同一个 **全局 90/分钟** 的闸门 —— 不分人、不分端点。后果是
> 反代卖着"多把 Key 叠加额度"却卡在 90/分钟，而且**网站一忙，反代跟着一起挂**。
> 实测压测时一半请求是 `Server is busy (global throttle)`。现在拆成独立一档。
